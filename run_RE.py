...
class InputRelations(object):
    """A single set of features of data."""

    def __init__(self, guid, name_1, name_2, psg, label):
        self.guid = guid
        self.name_1 = name_1
        self.name_2 = name_2
        self.psg = psg
...
class RelationExtraction(DataProcessor):
    """Processor for the RelationExtraction format which is a multi-class classification problem. where 1st column is name_1, 2nd is name_2, 3rd is passage, 4th is label."""
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            psg = line[2]
            label = line[3]
            examples.append(InputRelations(guid=guid, name_1=name_1, name_2=name_2, psg=psg, label=label))
        return examples
...
def convert_examples_to_relation_features(examples, label_list, max_seq_length, tokenizer):
    """Loads a data file into a list of `InputBatch`s."""
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        tokens_name_1 = tokenizer.tokenize(example.name_1)
        tokens_name_2 = tokenizer.tokenize(example.name_2)
        tokens_psg = tokenizer.tokenize(example.psg)
        # Modifies `tokens_name_1`, `tokens_name_2` and `tokens_psg` in place so that the total
        # length is less than the specified length.
...
def main():
    ...
    if args.do_train:
        train_features = convert_examples_to_relation_features(train_examples, label_list, args.max_seq_length, tokenizer)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        eval_features = convert_examples_to_relation_features(eval_examples, label_list, args.max_seq_length, tokenizer)
        ...
        for cur_epoch in range(int(args.num_train_epochs)):
            ...
            for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
                ...
                tmp_eval_accuracy, tmp_predict_list = metrics.accuracy(logits, label_ids)
                score_list.extend(tmp_predict_list.tolist())
            ...
            devset_filename = os.path.join(args.data_dir, "dev.tsv")
            score_filename = "score_" + str(cur_epoch) + ".tsv"
            output_score_file = os.path.join(args.output_dir, score_filename)
            with open(output_score_file, "w", encoding="utf8") as w:
                with open(devset_filename, "r", encoding="utf8") as f:
                    count = 0
                    for line in f:
                        row = line.strip()
                        write_line = row + "\t" + str(score_list[count]) + "\n"
                        w.write(write_line)
                        count += 1
