...
def run_evaluate(model, eval_features, device, eval_examples, tokenizer, best_dev_score=None):
    ...
    predict, nbest = write_predictions(eval_examples, eval_features, all_results, 5, 64, True, False)
    return predict
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    return nbest_json[0]["text"], all_nbest_json
...
eval_features = convert_examples_to_features(examples=examples, tokenizer=tokenizer, max_seq_length=250, doc_stride=128, max_query_length=64, is_training=False)  
device = torch.device("cpu")
res= run_evaluate(model, eval_features, device, examples, tokenizer=tokenizer)
print("res: ", res)