...
from BERT.modeling import BertConfig, BertForQuestionAnswering
...
def write_to_file(all_predictions, all_nbest_json, output_prediction_file, output_nbest_file):
    with open(output_prediction_file, "a+", encoding="utf8") as writer:
        writer.write(json.dumps(all_predictions, ensure_ascii=False, indent=4) + "\n")

    with open(output_nbest_file, "a+", encoding="utf8") as writer:
...
def run_evaluate(args, model, eval_features, device, eval_examples, tokenizer, predict_file, best_dev_score=None):
    ...
    with open(predict_file,encoding='utf-8') as dataset_file:
        dataset_json = json.load(dataset_file)
        dataset = dataset_json['data']
    dev_eval = evaluate(dataset, predict, tokenizer)
    dev_score = dev_eval['f1']
    dev_em = dev_eval['exact_match']
    dev_acc = dev_eval['acc']
    logger.info('Dev score : {}'.format(dev_score))
    logger.info('Dev em : {}'.format(dev_em))
    ...
    if best_dev_score is not None and dev_score > best_dev_score:
        ...
        write_to_file(predict, nbest, output_prediction_file, output_nbest_file)
    return best_dev_score, dev_score, dev_em, dev_acc
...
def main():
    ...
    bert_config_file = os.path.join(args.model_dir, "bert_config.json")
    vocab_file = os.path.join(args.model_dir, "vocab.txt")
    init_checkpoint = os.path.join(args.model_dir, "pytorch_model.bin")
    train_file = os.path.join(args.data_dir, "train.json")
    predict_file = os.path.join(args.data_dir, "dev.json")
    ...
    tokenizer = tokenization.BertTokenizer.from_pretrained(vocab_file, do_lower_case=args.do_lower_case)

    num_train_steps = None
    if args.do_train:
        logger.info('Load and process train examples')
        train_examples = read_squad_examples(input_file=train_file, tokenizer=tokenizer, is_training=True)
    ...
    if args.do_predict:
        ...
        eval_examples = read_squad_examples(
            input_file=predict_file, tokenizer=tokenizer, is_training=False)
    ...
    model_state_dict = torch.load(init_checkpoint, map_location="cpu")
    model = BertForQuestionAnswering.from_pretrained(args.model_dir, state_dict=model_state_dict)
    model.to(device)
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if (global_step + 1) % args.save_checkpoints_steps == 0:
                    best_dev_score, dev_score, dev_em, dev_acc = run_evaluate(args, model, eval_features, device, eval_examples, tokenizer, predict_file, best_dev_score)
                    logger.info('Best dev score {} in steps {}:'.format(best_dev_score, global_step))

                    with open(os.path.join(args.output_dir, "eval.tsv"), "a+", encoding="utf8") as eval_writer:
                        write_line = "best_dev_score: " + str(best_dev_score) + " dev_score: " + str(dev_score) + " dev_em: " + str(dev_em) + " dev_acc: " + str(dev_acc) + "\n"
                        eval_writer.write(write_line)
    ...
    if args.do_predict:
        ...
        model.to(device)        
        best_dev_score, dev_score, dev_em, dev_acc = run_evaluate(args, model, eval_features, device, eval_examples, tokenizer, predict_file)

        with open(os.path.join(args.output_dir, "test.tsv"), "a+", encoding="utf8") as test_writer:
            write_line = "best_test_score: " + str(best_dev_score) + " test_score: " + str(dev_score) + " test_em: " + str(dev_em) + " test_acc: " + str(dev_acc) + "\n"
            test_writer.write(write_line)
