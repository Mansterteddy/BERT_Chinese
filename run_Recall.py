...
def main():
    ...
    # Prepare model
    #model = BertForSequenceRelevance.from_pretrained(args.bert_model, device="cpu")
    model = BertForSequenceRelevance.from_pretrained(args.bert_model, device="cuda")
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for cur_epoch in range(int(args.num_train_epochs)):
            ...
            #model = BertForSequenceRelevance.from_pretrained(args.bert_model, state_dict=model_state_dict, device="cpu")
            model = BertForSequenceRelevance.from_pretrained(args.bert_model, state_dict=model_state_dict, device="cuda")
            model.to(device)
